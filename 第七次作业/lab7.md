...
- 参照lab6的最终思路，在上面利用全部学科的数据训练的**全局模型**的基础上，再利用分学科的数据对模型进行**微调**。（理性是美好的，但是事实表明居然是**反向优化**！！前两种思路的结果对比如下图所示）![image-20251110171339254](pictures/vs.png)
...
- **测试**集测试：
  ...
  在测试集上测试时只需调用提供的`regressor.predict()`方法即可，不过这样得到的是**放缩后的目标特征**（因为是用放缩过的y进行训练的），还需要利用`np.exp()`把目标特征**还原**

  ```py
  y_pred_scaled = regressor.predict(X_test_tabnet_scaled) # 测试
  y_pred = np.exp(y_pred_scaled).flatten() # 还原
  ```
...
- 模型目前存在的问题和未来可能的改进方向
  ...
  - **损失函数**问题，TabNet模型一个很重要的痛点是，只能选择mse作为损失函数，这其实不适合排名预测模型（使用MAE绝对误差作为损失函数可能更适合排名预测模型）搜索了一下，改进的话可以使用**TabNet 特征提取器**自己写训练循环，自由定义损失函数。
  - **在训练和验证集上表现很好，测试集却不佳**问题，数据泄露的问题应该没有，可能与上面目标特征的处理和损失函数的设置都有关系目标特征是被放缩过的，现在的变换方式会有”验证时差0.1，放回原尺度就差几十“的问题，使模型准确度下降。
